<div>
    ...
    <div class="flex justify-center">
        <button onclick="document.dispatchEvent(new CustomEvent('add-non-existing-product-button-pressed-shoppingList'))"
    ...
    <table class="mt-5 w-full text-xl">
        ...
        @foreach($products as $product)
            <tr class="border-b-2 @if($loop->first) border-t-2 @endif border-black text-center">
                <td>
                    <form action="{{ route('product.add-manual-existing-product-shoppinglist', ['id' => $product->id]) }}" method="POST">
                        @csrf
